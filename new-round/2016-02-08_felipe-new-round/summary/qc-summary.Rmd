...
This is the effect of MS vs non-MS:
...
heatmap_fn(log(counts[msaffected_genes,] + 1))
```

This is the specific effect of LPS treatment on the MS samples.
...
lpsmsaffected_genes = rownames(subset(lpsmseffect, padj < 0.05 & log2FoldChange > 2))
write.table(rownames(subset(lpsmseffect, padj < 0.05)), file="lpsms-effect.txt",
            row.names=FALSE, col.names=FALSE, quote=FALSE)
heatmap_fn(log(counts[lpsmsaffected_genes,] + 1))
```

This is the specific effect of LPS+IL27 treatment on the MS samples.

```{r effect-of-LPS+IL27}
il27lpseffect = results(dds, name="phenotypeMS.treatmentLPS.IL27")
```
...
```{r DESeq-tables, results='asis', cache=FALSE}
for(i in seq(length(all_results))) {
